...
Six step kinds (full constraints in [dsl-schema.json](resources/dsl-schema.json)):

| Kind | Purpose | When to Use |
|------|---------|------------|
| `call` | Invoke a native MCP tool | Each upstream API call |
| `http` | Call an external HTTP API on a named `backend` upstream | Direct HTTP calls to APIs not covered by native tools |
| `jq` | jq expression transformation (`from` + `expr` + `vars`) | Reshape fields, filter, project, construct new objects after a `call` or `http` |
...
> **IMPORTANT** — This (current) MCP server is generated by MCPFather from an OpenAPI spec and may be regenerated as the upstream API evolves. **Never modify generated Go source code.** The pipeline DSL + jq is your full implementation surface — `call`, `http`, `jq`, `foreach`, `emit`, `return`, and jq's built-ins (map, select, reduce, conditionals, string interpolation, etc.) are expressive enough for any orchestration need. If you think you need Go code, push harder on the pipeline first.
